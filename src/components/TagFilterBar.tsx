 ...
 *
 * The filter state lives in the parent (App) via useTagFilter — this is purely
 * presentational plus hotkey wiring.
 */
...
import { TAG_HOTKEYS } from '../hotkeys';
...
export function TagFilterBar({
  ...
  /* Tags sorted by activity count (most popular first) — used for both
     display ordering and assigning digit hotkeys */
  ...
  /* ------ Digit hotkeys for top-9 tags ------ */
  /* React rules forbid hooks in loops, so we inline 9 calls. Each one
     is enabled only when the corresponding tag exists and we're not in
     a text input (useHotkey handles that internally). */
  useHotkey(TAG_HOTKEYS[0].code, () => sortedTags[0] && onToggleTag(sortedTags[0].name), sortedTags.length >= 1);
  useHotkey(TAG_HOTKEYS[1].code, () => sortedTags[1] && onToggleTag(sortedTags[1].name), sortedTags.length >= 2);
  useHotkey(TAG_HOTKEYS[2].code, () => sortedTags[2] && onToggleTag(sortedTags[2].name), sortedTags.length >= 3);
  useHotkey(TAG_HOTKEYS[3].code, () => sortedTags[3] && onToggleTag(sortedTags[3].name), sortedTags.length >= 4);
  useHotkey(TAG_HOTKEYS[4].code, () => sortedTags[4] && onToggleTag(sortedTags[4].name), sortedTags.length >= 5);
  useHotkey(TAG_HOTKEYS[5].code, () => sortedTags[5] && onToggleTag(sortedTags[5].name), sortedTags.length >= 6);
  useHotkey(TAG_HOTKEYS[6].code, () => sortedTags[6] && onToggleTag(sortedTags[6].name), sortedTags.length >= 7);
  useHotkey(TAG_HOTKEYS[7].code, () => sortedTags[7] && onToggleTag(sortedTags[7].name), sortedTags.length >= 8);
  useHotkey(TAG_HOTKEYS[8].code, () => sortedTags[8] && onToggleTag(sortedTags[8].name), sortedTags.length >= 9);
    ...
    <div className={`tag-filter-bar${filterOn ? ' tag-filter-bar-active' : ''}`}>
      <div className="tag-filter-top-row">
        ...
        {multiTagActive && (
          <button
            ...
            title={filterMode === 'OR'
              ? 'Currently OR — activities matching ANY active tag. Click to switch to AND (must match ALL)'
              : 'Currently AND — activities must match ALL active tags. Click to switch to OR (any tag)'}
          >
            {filterMode}
          </button>
        )}
      ...
      <div className="tag-filter-pills" role="group" aria-label="Tag filters">
        ...
        {displayedTags.map((tag) => {
          ...
          const globalIdx = sortedTags.indexOf(tag);
          const hotkey = globalIdx >= 0 && globalIdx < 9 ? TAG_HOTKEYS[globalIdx].label : null;
          const isActive = activeTags.includes(tag.name);

          const pillStyle = tag.color && isActive
          ...
          return (
            <button
              ...
              title={hotkey ? `Toggle "${tag.name}" filter (${hotkey})` : `Toggle "${tag.name}" filter`}
            >
              {tag.name}
