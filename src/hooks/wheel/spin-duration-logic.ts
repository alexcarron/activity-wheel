...
/**
 * Compute and return the timing for the next spin based on the provided spinCount. The caller is responsible for managing and storing the spinCount state. Call this exactly once per spin, just before triggering the animation. 
 */
export function getNextSpinTiming(currentSpinCount: number): SpinTimingResult {
	...
	const expectedDurationMS = Math.max(MIN_SPIN_DURATION_MS, calculatedDuration);

	const jitterRangePercentage = withMinAndMax(
		getPercentageOfValueBetweenNumbers(
			expectedDurationMS,
			MIN_SPIN_DURATION_MS,
			INITIAL_SPIN_DURATION_MS,
		),
		0,
		1,
	);
