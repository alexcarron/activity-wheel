 ...
 *
 * Each wheel is a named namespace for a set of activities + tags.
 * The active wheel ID is persisted in localStorage (UI preference state).
 */
...
import { clearWheelTagMetadata, copyTagMetadata, ensureTagsExist, listTagMetadata } from './tag-service';
...
export function getStoredActiveWheelId(): string {
  return localStorage.getItem(ACTIVE_WHEEL_KEY) ?? 'default';
}

export function persistActiveWheelId(id: string): void {
...
export async function listWheels(): Promise<Wheel[]> {
  ...
  return all.sort((a, b) => b.lastUsedAt - a.lastUsedAt || a.createdAt - b.createdAt);
}
...
/**
 * Delete a wheel and all its activities + tag metadata.
 * The caller must ensure this is not the last wheel.
 */
...
/**
 * Import a full backup. Deletes ALL existing wheels and replaces them with the
 * wheels in the file. Returns the ID of the first wheel in the backup so the
 * caller can switch to it.
 */
export async function importFullBackup(json: string): Promise<string> {
	...
	if (isLegacyDbDump(parsed)) {
		await db.importAll(parsed as import('../libraries/indexeddb').DBBackup);
		const wheels = await listWheels();
		return wheels[0]?.id ?? 'default';
	}

	if (!isFullBackup(parsed)) {
		const asObj = typeof parsed === 'object' && parsed !== null
	...
	const wStore = db.store<Wheel>(WHEELS_STORE.name);
	const tStore = db.store<TagMetadata>(TAG_METADATA_STORE.name);
	for (const { wheel, activities, tags } of parsed.wheels) {
		await wStore.put(wheel);
		if (activities.length > 0) await bulkPut(activities);
		for (const tag of tags) await tStore.put(tag);
	}
...
function isFullBackup(v: unknown): v is FullBackup {
	if (typeof v !== 'object' || v === null) return false;
	const obj = v as Record<string, unknown>;
	return obj.format === 'full-backup-v2' && Array.isArray(obj.wheels);
}

function isLegacyDbDump(v: unknown): boolean {
	if (typeof v !== 'object' || v === null) return false;
	const obj = v as Record<string, unknown>;
...
export async function copyWheel(
  ...
  for (const a of sourceActivities) {
    await addActivity(a.name, newWheel.id, now);
  ...
  if (!resetWeights && sourceActivities.length > 0) {
    ...
    const nameToSource = new Map(sourceActivities.map((a) => [a.name, a]));
    await bulkPut(
      created.map((a) => {
        const src = nameToSource.get(a.name);
        if (!src) return a;
        return {
          ...a,
          weight: src.weight,
          acceptCount: src.acceptCount,
          rejectCount: src.rejectCount,
          streak: src.streak,
          tags: src.tags,
    ...
    await bulkPut(
      created.map((a) => {
        const src = nameToSource.get(a.name);
        return src ? { ...a, weight: WEIGHT_DEFAULT, tags: src.tags } : a;
      }),
    );
  }
  ...
  const allTags = [...new Set(sourceActivities.flatMap((a) => a.tags ?? []))];
