 ...
 *
 * This is the only place the activity wheel app talks to the generic IDB
 * library about schema. New schema versions get added here as additive
 * migrations — never modify a previous one in place.
 *
 * Version history:
 *  v1 — initial: activities store
 *  v2 — tagging: tag-metadata store (tag name → optional color)
 *  v3 — multi-wheel: wheels store; wheelId on activities; tag-metadata
 *        re-keyed to "${wheelId}:${tagName}" with keyPath changed to 'key'.
 *        Existing activities → wheelId='default'; existing tag colors preserved.
 */

import type { DBConfig, Migration, StoreSchema } from '../libraries/indexeddb';
  ...
  apply: ({ db, transaction }) => {
    ...
    if (db.objectStoreNames.contains('tag-metadata')) {
      ...
      tagCursor.onsuccess = (e) => {
        const cur = (e.target as IDBRequest<IDBCursorWithValue | null>).result;
        if (cur) {
          saved.push(cur.value as { name: string; color?: string });
          cur.continue();
        } else {
          ...
          for (const rec of saved) {
            const entry: Record<string, unknown> = {
              key: `default:${rec.name}`,
              wheelId: 'default',
              name: rec.name,
            };
            if (rec.color) entry.color = rec.color;
            newStore.put(entry);
          }
        }
      };
