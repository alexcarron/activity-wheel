/**
 * Effective weight. The value actually used for selection probabilities and UI display.
 * The recency boost is context-aware when pool info is available, so new activities scale appropriately against the existing crowd.
 * Always returns at least WEIGHT_MIN so every activity has a non-zero chance of being selected. Nothing is permanently unspinnable.
 */
...
/**
 * Gets the effective weight used for wheel selection and probability display. Optionally accepts pool context to compute a more accurate recency boost. 
 */
