 ...
 *
 * Rationale: as a weight approaches its bound (MAX for positive changes,
 * MIN for negative changes), the effective step size shrinks. This prevents
 * weights from slamming hard into the ceiling or floor, and makes the curve
 * feel natural — early feedback has more impact than feedback at extremes.
 *
 * Formula:
 *   ratio    = distance_to_bound / (MAX − MIN)        [0 = at bound, 1 = far away]
 *   factor   = max(FLOOR, ratio ^ EXPONENT)
 *
 * At the midpoint from either bound, factor ≈ 0.62 (with exponent 0.7).
 * At the bound itself, factor = FLOOR (default 0.1) — a tiny nudge still fires.
 */
