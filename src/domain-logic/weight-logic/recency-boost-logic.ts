 ...
 *
 * Purpose: ensure new activities actually get sampled in the first week
 * without requiring the user to manually boost them.
 *
 * Formula:
 *   boost = baseBoost × (1 − days / RECENCY_DAYS)   [linear fade to 0]
 *
 * baseBoost is derived from pool context when available:
 *   baseBoost = poolAvgWeight × RECENCY_BOOST_MULTIPLIER
 *
 * This means the bonus scales with the rest of the pool — if existing
 * activities have drifted high through lots of accepts, the new activity
 * still gets a proportionally competitive head-start.
 *
 * Falls back to RECENCY_BOOST_AVG_WEIGHT_FALLBACK (flat points) when no pool context
 * is supplied (first-ever activity, unit tests, debug panel).
 */
...
/**
 * Returns the recency bonus (in weight points) for `activity` at time `now`.
 * Returns 0 once the activity is older than RECENCY_DAYS.
 */
export function getRecencyWeightBoost(
  ...
  if (
    ...
    const avgWeight = globalWeightContext.totalEffectiveWeight / globalWeightContext.numTotalActivities;
